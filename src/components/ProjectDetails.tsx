...
function SectionHeader({ icon: Icon, label }: { icon: React.ElementType; label: string }) {
  return (
    <div className="flex items-center gap-2 mb-3">
      ...
      <span className="font-mono text-cyber-yellow text-xs uppercase tracking-widest">{label}</span>
  ...
  return (
    <AnimatePresence>
      ...
      <motion.div
        ...
        className="fixed top-0 right-0 h-full w-full max-w-2xl z-[9999] bg-[#0a0018] border-l border-cyber-yellow/30 flex flex-col overflow-hidden"
      >
        {/* Header */}
        <div className="flex items-start justify-between p-6 border-b border-cyber-yellow/20 flex-shrink-0">
          <div>
            <span className="font-mono text-cyber-yellow text-xs uppercase tracking-widest block mb-1">
            ...
            <div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-800 bg-cyber-yellow/5">
                      <th className="text-left px-3 py-2 font-mono text-gray-500 text-xs uppercase tracking-wider">Component</th>
                      <th className="text-center px-3 py-2 font-mono text-gray-500 text-xs uppercase tracking-wider w-16">Qty</th>
                      <th className="text-right px-3 py-2 font-mono text-gray-500 text-xs uppercase tracking-wider w-24">Unit Cost</th>
                    </tr>
                  </thead>
                  <tbody>
              ...
              {details.totalCost && (
                <div className="mt-2 flex justify-end">
                  <span className="font-mono text-xs text-gray-500 uppercase tracking-wider mr-2">Total:</span>
        ...
        <div className="flex-shrink-0 p-4 border-t border-cyber-yellow/20">
          <button
            onClick={onClose}
            className="w-full py-2.5 font-mono text-xs uppercase tracking-widest border border-gray-700 text-gray-400 hover:border-cyber-yellow hover:text-cyber-yellow transition-colors"
          >
            Close
          </button>
        </div>
